...
SELECT 
    ...
    SUM(CASE
        ...
        ELSE 0
    END) AS WORLD_WIDE_GROSS_NULL_COUNT,
    SUM(CASE
...
SELECT 
    COUNT(DISTINCT id) AS number_of_movies_in_INDIA_or_USA, year
FROM
    movie
WHERE
    (country LIKE '%INDIA%'
...
WITH X AS (
SELECT genre, COUNT(*) as cnt, RANK() OVER(ORDER BY COUNT(*) desc) AS grank FROM genre GROUP BY genre
)
SELECT genre, cnt from X WHERE grank=1;

-- using rank because it takes 0.001 sec less time
...
SELECT * FROM top_avg_rating WHERE movie_rank<=10;
...
SELECT
CASE 
	WHEN (SELECT total_votes FROM votes_view WHERE country = "Germany") > (SELECT total_votes FROM votes_view WHERE country = "ITALY") THEN "YES"
	ELSE "NO"
END AS ANSWER;
*/

SELECT 
    m.country AS COUNTRY, COUNT(r.total_votes) AS total_votes
FROM
    ratings AS r
        INNER JOIN
...
GROUP BY actor_name, category
)
SELECT actor_name, movie_count FROM top_actors WHERE actor_rank<=2;
...
SELECT * FROM top_prod_comp WHERE prod_comp_rank<=3;
...
SELECT * FROM top_actresses WHERE actress_rank<=5;
...
-- some of the values for gross income is in INR. We need to convert it into USD and then srt it by numerical value of USD

update movie
set worlwide_gross_income = concat("$ ",CONVERT(ROUND(CONVERT(substring(worlwide_gross_income,5), DECIMAL)*0.012), CHAR ))
...
top5movies as 
(
SELECT genre, m.year as year, m.title as movie_name, worlwide_gross_income,
			DENSE_RANK() OVER(PARTITION BY year ORDER BY CONVERT(substring(worlwide_gross_income,5), DECIMAL) DESC) as movie_rank
FROM movie as m
	INNER JOIN genre as g
		ON g.movie_id = m.id
WHERE genre in (SELECT genre FROM top3genres)
)
SELECT * 
FROM top5movies 
WHERE movie_rank<=5
...
-- Using dense rank so that all actresses are considered if needed. 
-- Doing this because we don't know how to order actresses with same superhit movie count.

WITH actress_rank_view AS 
(
...
SELECT name_id AS director_id,
    ...
    RANK() OVER(ORDER BY COUNT(movie_id) DESC) AS dirRank
FROM top9directors
